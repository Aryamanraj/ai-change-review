...
import { AcceptedHunk, FileRecord, ReviewHunk } from "./types";
...
export class ReviewPanel implements vscode.Disposable {
  ...
  private async handle(message: { type?: string; hunkId?: string }): Promise<void> {
    ...
    if (!current) { return; }
    try {
      if (message.type === "keep" && message.hunkId) { await this.manager.acceptHunk(current, message.hunkId); }
      if (message.type === "undo" && message.hunkId) { await this.manager.rejectHunk(current, message.hunkId); }
      if (message.type === "keepFile") { await this.manager.accept(current); }
  ...
  private async render(): Promise<void> {
    ...
    this.panel.webview.html = this.html(content.current, content.hunks, content.accepted, this.title(current), current.label, current.addedLines ?? 0, current.removedLines ?? 0);
  }
  ...
  private html(current: string, hunks: ReviewHunk[], accepted: AcceptedHunk[], heading: string, label: string, added: number, removed: number): string {
    const nonce = String(Date.now());
    const blocks = this.fullFile(current, hunks, accepted);
    const changeCount = hunks.length + accepted.length;
  ...
  private hunk(hunk: (ReviewHunk | (AcceptedHunk & { currentStart: number })) & { state: "pending" | "accepted" }, index: number, total: number): string {
    ...
    const controls = hunk.state === "accepted"
      ? `<span class="accepted-label">✓ Kept</span>`
      : `<button data-action="keep" data-hunk="${hunk.id}">Keep</button><button class="undo" data-action="undo" data-hunk="${hunk.id}">Undo</button>`;
