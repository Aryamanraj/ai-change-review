...
const KEYWORDS = new Set("abstract any as assert async await break case catch class const continue debugger declare default delete do else enum export extends finally for from function get if implements import in infer instanceof interface is keyof let namespace new of override private protected public readonly return satisfies set static super switch throw try type typeof undefined using var void while with yield".split(" "));
const LITERALS = new Set("true false null undefined".split(" "));
...
export class ReviewPanel implements vscode.Disposable {
  ...
  private async handle(message: { type?: string; hunkId?: string }): Promise<void> {
    ...
    try {
      if (message.type === "keep" && message.hunkId) {
        ...
        await this.advanceToNextPending();
      }
  ...
  private async advanceToNextPending(): Promise<void> {
    ...
    // The next change is in this file (or there are no remaining changes). Rebuild
    // the document so accepted code becomes plain final code and focus moves on.
    await this.render();
  }
  ...
  private html(current: string, hunks: ReviewHunk[], heading: string, label: string, added: number, removed: number): string {
    ...
    const actions = changeCount ? `<button data-action="keepFile">Keep file</button><button class="undo" data-action="undoFile">Undo file</button><button class="nav" data-nav="previous" aria-label="Previous change">↑</button><button class="nav" data-nav="next" aria-label="Next change">↓</button><span class="summary" id="change-count">1 of ${changeCount}</span>` : `<span class="summary">Final code</span>`;
    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'nonce-${nonce}'; script-src 'nonce-${nonce}';"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style nonce="${nonce}">
      ...
      .tok-keyword { color: var(--vscode-symbolIcon-keywordForeground, #c586c0); } .tok-string { color: var(--vscode-debugTokenExpression-string, #ce9178); } .tok-number, .tok-literal { color: var(--vscode-debugTokenExpression-number, #b5cea8); } .tok-type { color: var(--vscode-symbolIcon-classForeground, #4ec9b0); } .tok-comment { color: var(--vscode-editorCodeLens-foreground, #6a9955); font-style: italic; }
    </style></head><body><header><div><h1>${escapeHtml(label)}</h1><div class="summary">${escapeHtml(heading)} · <span class="add">+${added}</span> <span class="remove">−${removed}</span></div></div><div class="header-actions">${actions}<button id="open-current">Open current file</button></div></header><main>${blocks || this.plain(current.split("\n"), 1)}</main><script nonce="${nonce}">const vscode = acquireVsCodeApi(); document.getElementById('open-current').addEventListener('click', () => vscode.postMessage({type:'openCurrent'})); document.querySelectorAll('[data-action]').forEach(button => button.addEventListener('click', () => vscode.postMessage({type: button.dataset.action, hunkId: button.dataset.hunk}))); const cards=[...document.querySelectorAll('.hunk')]; let index=0; const counter=document.getElementById('change-count'); const go=step=>{if(!cards.length)return; index=(index+step+cards.length)%cards.length; cards[index].scrollIntoView({behavior:'smooth',block:'start'}); if(counter)counter.textContent=(index+1)+' of '+cards.length;}; document.querySelectorAll('[data-nav]').forEach(button=>button.addEventListener('click',()=>go(button.dataset.nav==='next'?1:-1))); if(cards.length){setTimeout(()=>cards[0].scrollIntoView({block:'center'}),0);}</script></body></html>`;
  }
  ...
  private plain(lines: string[], start: number): string {
    ...
    return `<pre class="plain">${lines.map((line, index) => `<div class="line"><span class="number">${start + index}</span><span class="marker"> </span><span>${this.highlight(line)}</span></div>`).join("")}</pre>`;
  }
  ...
  private hunk(hunk: ReviewHunk & { state: "pending" }, index: number, total: number): string {
    ...
    const lines = hunk.lines.map(line => {
      const prefix = line[0]; const text = this.highlight(line.slice(1));
  ...
  /** Small dependency-free tokenizer for the webview; colors follow the active VS Code theme. */
  private highlight(line: string): string {
    ...
    for (const match of line.matchAll(token)) {
      ...
      output += kind ? `<span class="tok-${kind}">${escapeHtml(value)}</span>` : escapeHtml(value);
